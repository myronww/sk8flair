...
export TARGETHOST=sk8s-daronwalker-r.local

echo "Deploying to target host ($TARGETHOST)"

ssh-keygen -R sk8s-daronwalker-r.local 
...
scp ./bluetooth/daronwalker-right.conf root@$TARGETHOST:/etc/bluetooth/main.conf
