...
export TARGETHOST=sk8s-myronwalker-r.local

echo "Deploying to target host ($TARGETHOST)"

ssh-keygen -R sk8s-myronwalker-r.local 
ssh-keygen -R 192.168.42.1

ssh root@$TARGETHOST 'rm -fr ~/sk8flair && rm -fr ~/src && mkdir -p ~/sk8flair/profiles && mkdir -p ~/bin && mkdir -p ~/tests && mkdir -p ~/src'

# Deploy the custom bluetooth configuration files
scp ./bluetooth/myronwalker-right.conf root@$TARGETHOST:/etc/bluetooth/main.conf
...
scp ../bin/*.* root@$TARGETHOST:/home/root/bin/
