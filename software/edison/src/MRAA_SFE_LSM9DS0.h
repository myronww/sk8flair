...
#define DEFAULT_XM_ADDRESS (uint8_t)0x1D
#define DEFAULT_GYRO_ADDRESS (uint8_t)0x6B
...
public:
    ...
    int16_t acc_center_bias[3];
	int16_t gyro_center_bias[3];
    int16_t mag_center_bias[3];   //Centering bias

    float mag_scale_bias[3];   //Scale bias

    // Calibration In Process Values
    int16_t acc_avg_pos_axis[3];
    int16_t gyro_avg_pos_axis[3];
    int16_t mag_avg_pos_axis[3];

    int16_t mag_avg_rad_axis[3];
	...
	int32_t calibrate_x(uint64_t time_ms, uint64_t period_ms, int16_t pos_center, int16_t pos_lower, int16_t pos_upper);

	int32_t calibrate_y(uint64_t time_ms, uint64_t period_ms, int16_t pos_center, int16_t pos_lower, int16_t pos_upper);

	int32_t calibrate_z(uint64_t time_ms, uint64_t period_ms, int16_t pos_center, int16_t pos_lower, int16_t pos_upper);
	...
	// readMotion() -- Read all the motion related output registers.
	// This function will read all of the motion related values from the registers.
	// This includes six accelerometer output registers, six gyroscope output
	// registers, and six magnetometer output registers.
	// The readings are stored in the class' ax, ay, az, gx, gy, gz, mx, my and mz
	// variables. Read those _after_ calling readMotion().
	void readMotion(motion_event* event=NULL, bool adjust_output=false);
	...
	void calcMotion(motion_event* event, f_motion_event* fevent);
...
private:
	...
	int32_t calibrate_axis(uint64_t time_ms, uint64_t period_ms, int16_t pos_center, int16_t pos_lower, int16_t pos_upper,
	...
	inline int16_t decode_as_int16(uint8_t bH, uint8_t bL) {
		int16_t ival = (bH & 0x80) == 0 ? (bH << 8 | bL) : (((((bH << 8) | bL) ^ 0xffff) & 0xffff) + 1) * -1;
		return ival;
	}
